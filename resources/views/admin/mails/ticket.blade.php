                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="ticket-message" class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    <a class="btn btn-outline-primary mr-1 mb-2" href="{{ url('/admin/mails/contact') }}">Contact Messages</a>
                                    <a class="btn btn-primary text-white mb-2" href="{{ url('/admin/mails/ticket') }}">Tickets</a>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    <div class="card m-b-0">
                                        <div class="card-header" id="headingOne">
                                            <a class="row collapsed" data-toggle="collapse" data-target="#ticket-{{$message->id}}" aria-expanded="false" aria-controls="ticket-{{$message->id}}">
                                                <span class="col-3 pl-5">{{$message->users->fname}}</span>
                                                <span class="col-7">{{$message->subject}} - {{$message->content}}</span>
                                                <span class="col-2">{{$message->created_at}}</span>
                                            </a>
                                        </div>
                                        <div id="ticket-{{$message->id}}" class="collapse" aria-labelledby="headingOne" data-parent="#accordionExample">
                                            <div class="card-body">
                                                <div class="mb-2">
                                                    <small>from</small>
                                                    <span class="badge badge-pill badge-warning"> {{$message->users->email}}</span>
                                                </div>
                                                <h5>{{$message->subject}}</h5>
                                                <p>{{$message->content}}</p>
                                                <button type="button" class="btn btn-sm btn-outline-primary btn-modal" data-value="{{ $message->id }}" data-email="{{ $message->users->email }}" data-toggle="modal" data-target="#messageModal">Reply</button>
                                            </div>
                                        </div>
                                    </div>
                                </tr>
